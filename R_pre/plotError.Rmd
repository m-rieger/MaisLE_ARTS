...
for(m in unique(df$meth[df$site == "maisC"])) {
  ...
  for(a in unique(dat$AcSc)) {

    print(ggplot(dat[dat$AcSc == a,]) +
      ...
      geom_smooth(aes(x = dens, y = dist, group = detR, color = detR), method = "glm", formula = y ~ poly(x, 2), data = dat.m[dat.m$AcSc == a,], lty = "dashed") +
      ggtitle(paste0(m, " - ", a), subtitle = "position error (solid), position error from mean positions (dashed)") +
...
for(m in unique(df$meth[df$site == "maisD"])) {
  dat <- df[df$site == "maisD" & df$meth == m & df$ant == "1-8",]
  dat.m <- df.m[df.m$site == "maisD" & df.m$meth == m & df.m$ant == "1-8",]
...
for(m in unique(df$meth[df$site == "maisC"])) {
  ...
  for(a in unique(dat.m$AcSc)) {

    print(ggplot(dat.m[dat.m$AcSc == a,]) +
      geom_point(aes(x = dens, y = dist, color = detR, group = detR),
                 pch = 1, alpha = 0.2,
                 position = position_dodge(width = 0.5)) +
      geom_smooth(aes(x = dens, y = dist, group = detR, color = detR), method = "glm", formula = y ~ poly(x, 2), lty = "dashed") +
      ggtitle(paste0(m, " - ", a), subtitle = "position error from mean positions (dashed)") +
      ylab("position error [m]") +
      ylim(0, NA) +
      facet_wrap(~Individual, scales = "free_y") +
      scale_color_viridis_d() +
...
for(m in unique(shp$meth[shp$site == site])) {
  for(d in unique(shp$detR[shp$site == site & shp$meth == m])) {
    if(m %in% c("ab_ml", "ml_ml")) type <- "omni"
    if(m %in% c("ab_ql", "in_ql")) type <- "direct"
    
    g <- ggplot() + 
      geom_sf(data = shp[shp$site == site & shp$meth == m & shp$detR == d,], 
...
for(m in unique(shp.m$meth[shp.m$site == site])) {
  for(d in unique(shp.m$detR[shp.m$site == site & shp.m$meth == m])) {
    ...
    g <- ggplot() + 
      geom_sf(data = shp.m[shp.m$site == site & 
...
for(m in unique(shp.t$meth[shp.t$site == site])) {
  for(d in unique(shp.t$detR[shp.t$site == site & shp.t$meth == m])) {
    ...
    g <- ggplot() + 
      geom_sf(data = shp.t[shp.t$site == site & 
