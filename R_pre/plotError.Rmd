...
for(m in unique(df$meth[df$site == "maisC"])) {
  ...
  print(ggplot(dat) +
    ...
    ggtitle(paste0(m), subtitle = "position error (solid)") +
...
for(m in unique(df$meth[df$site == "maisC"])) {
  dat <- df[df$site == "maisC" & df$meth == m & df$ant == "1-10",]
    ...
    print(ggplot(dat) +
      ...
      ggtitle(paste0(m), subtitle = "position error from est. positions (dashed)") +
      ylab("position error [m]") +
      ylim(0, NA) +
      facet_wrap(~Individual, scales = "free_y") +
      scale_color_viridis_d() +
...
# raster
These plots show summarized raster (`r rast_res*rast_inc`x`r rast_res*rast_inc` m resolution) based on positions from testtags with ground truth data (GPS data). Each plot is split by individual (= testtag, e.g. TT090C) and separate plots were made per method (*direct.ab* = antenna beams directional, *omni.ab* = antenna beams omnidirectional, *omni.ml* = mulitlateration omnidirectional, *direct.in* = intersection directional)  
...
for(m in unique(shp$meth[shp$site == site])) {
  for(d in unique(shp$detR[shp$site == site & shp$meth == m])) {
    ...
    if(m %in% c("direct.ab", "direct.in")) type <- "direct"
