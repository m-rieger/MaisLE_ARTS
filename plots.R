...
df.pred2 <- read.csv(here("output_model", "model-predictions_m2.csv"))
df.sim2  <- read.csv(here("output_model", "model-simulations_m2.csv"))

df.pred3 <- read.csv(here("output_model", "model-predictions_m3.csv"))
df.sim3  <- read.csv(here("output_model", "model-simulations_m3.csv"))

## model/method validation
df.meth <- data.frame(meth = c("direct.ab", "direct.ab", "direct.an", "direct.an", "omni.ab", "omni.ml"),
...
Lm <- Ld <- list()
for(i in df.meth$ID) {
  Lm[[i]] <- readRDS( here("output_model", paste0("model_m3_", i, ".RDS"))) 
  Ld[[i]] <- read.csv(here("output_model", paste0("data_m3_", i, ".csv"))) 
}
...
fileL <- list.files(here("output_model"), pattern = "data_test_m3")
df.t3 <- data.frame()
for(f in fileL) df.t3 <- rbind(df.t3, read.csv(here("output_model", f)))
...
write.csv(sum.p, "./plots/tabTesttracks.csv", row.names = F)            
...
df.m2 <- df.m2 %>% group_by(X_time, tagID, site) %>%
  mutate(Nmeth = length(unique(meth)),
         nP = n()) %>%
  ungroup()

df.m2$allM <- "no"
df.m2$allM[df.m2$site == "maisC" & df.m2$Nmeth == 4] <- "yes"
df.m2$allM[df.m2$site == "maisD" & df.m2$Nmeth == 2] <- "yes"

df.m2$allM <- factor(df.m2$allM, levels = c("no", "yes"))
df.m2$allM2 <- factor(df.m2$allM, levels = c("yes", "no"))
df.m2$allM.site <- factor(paste0(df.m2$allM, " ", df.m2$site), levels = c("yes maisC", "no maisC", "yes maisD", "no maisD"))

## add column to indicate whether position is kept due to thresholds
df.m2$thresh <- "yes"
df.m2$thresh[df.m2$meth == "direct.ab" & df.m2$site == "maisC" & df.m2$Ac == df.m2$Sc] <- "no"
df.m2$thresh[df.m2$meth == "direct.ab" & df.m2$site == "maisD" & df.m2$Ac <= 3 & df.m2$Sc %in% c(1, 2, 3)] <- "no"
df.m2$thresh[df.m2$meth == "omni.ab" & df.m2$site == "maisC" & df.m2$Ac == 1] <- "no"

## add several IDs for plotting and merging
df.m2$group <- paste0(df.m2$site, " \n", df.m2$meth)
df.m2$ID <- paste0(df.m2$site, "_", df.m2$meth)
df.m2$meth2 <- df.m2$meth
df.m2$meth2[df.m2$meth2 == "direct.ab"] <- "direct ab"
df.m2$meth2[df.m2$meth2 == "direct.an"] <- "direct an"
df.m2$meth2[df.m2$meth2 == "omni.ab"] <- "omni ab"
df.m2$meth2[df.m2$meth2 == "omni.ml"] <- "omni ml"
df.m2$meth3 <- df.m2$meth
df.m2$meth3[df.m2$meth3 == "direct.ab"] <- "direct \nab"
df.m2$meth3[df.m2$meth3 == "direct.an"] <- "direct \nan"
df.m2$meth3[df.m2$meth3 == "omni.ab"] <- "omni \nab"
df.m2$meth3[df.m2$meth3 == "omni.ml"] <- "omni \nml"
df.m2$group2 <- paste0(df.m2$meth3, " \n", df.m2$site)

# get n per group
df.m2 <- df.m2 %>% group_by(site, allM2, meth) %>%
  mutate(N = n()) %>% ungroup()

df.sim1$r <- factor(df.sim1$r, ordered = T)

tmp.m1 <- subset(df.sim1, model == "m1" & meth == "direct.ab")
tmp.m2 <- subset(df.sim2, model == "m2")
tmp.m2$meth2 <- tmp.m2$meth
tmp.m2$meth2[tmp.m2$meth2 == "direct.ab"] <- "direct ab"
tmp.m2$meth2[tmp.m2$meth2 == "direct.an"] <- "direct an"
tmp.m2$meth2[tmp.m2$meth2 == "omni.ab"] <- "omni ab"
tmp.m2$meth2[tmp.m2$meth2 == "omni.ml"] <- "omni ml"
tmp.m2$meth3 <- tmp.m2$meth
tmp.m2$meth3[tmp.m2$meth3 == "direct.ab"] <- "direct \nab"
tmp.m2$meth3[tmp.m2$meth3 == "direct.an"] <- "direct \nan"
tmp.m2$meth3[tmp.m2$meth3 == "omni.ab"] <- "omni \nab"
tmp.m2$meth3[tmp.m2$meth3 == "omni.ml"] <- "omni \nml"
tmp.m2$group2 <- paste0(tmp.m2$meth3, " \n", tmp.m2$site)
#tmp.m2 <- subset(df.pred, model == "m2" & meth == "ab_ql")

df.pred3$diffCI2 <- ifelse(df.pred3$diffCI <= 10, "0-10",
                          ifelse(df.pred3$diffCI <= 25, "10-25",
                                ifelse(df.pred3$diffCI <= 50, "25-50",
                                       ifelse(df.pred3$diffCI <= 75, "50-75", paste0("75-", floor(max(df.pred3$diffCI)))))))
df.pred3$diffCIq502 <- ifelse(df.pred3$diffCIq50 <= 10, "0-10",
                          ifelse(df.pred3$diffCIq50 <= 25, "10-25",
                                 ifelse(df.pred3$diffCIq50 <= 50, "25-50",
                                        ifelse(df.pred3$diffCIq50 <= 75, "50-75", paste0("75-", floor(max(df.pred3$diffCIq50)))))))

## get prop Points per method
df.points <- df.m2 %>% group_by(site, meth, group2, allM2) %>%
...
g1 <- ggplot(tmp.m1) +
  stat_halfeye(aes(x = r, y = sim.m, group = site, color = site, fill = site,
...
g4PE <- ggplot() +
  stat_eye(.width = NA, adjust = 3, 
           #point_interval = NULL,
           data = df.m2, aes(x = group2, y = PE, fill = site, color = site, alpha = allM2,
...
g <- ggplot(df.pred3[df.pred3$meth == "direct.ab",]) + 
  # geom_point(aes(x = Ac, y = PE, group = as.factor(Sc), color = as.factor(Sc)),
  #            data = df.m2[df.m2$meth2 == "direct \nab",],
...
ggsave("./plots/plotPA2.pdf", plot = g, width = 34, height = 12, device = "pdf", units = "cm")

df.sim3$Sc <- as.factor(df.sim3$Sc)
df.pred3$Sc <- as.factor(df.pred3$Sc)

## correlations between coefficients
# gPE <- ggplot(df.pred3) + 
#   geom_point(aes(x = Ac, y = PE, group = as.factor(Sc), color = as.factor(Sc)),
#              data = df.m2[df.m2$meth3 == "direct \nab",],
...
# 
# 
# gcw <- ggplot(df.m2[df.m2$meth3 == "direct \nab",]) + 
...
# 
# gsw <- ggplot(df.m2[df.m2$meth3 == "direct \nab",]) + 
...
# 
# gsc <- ggplot(df.m2[df.m2$meth3 == "direct \nab",]) + 
...
df.t3 <- df.t3 %>% group_by(X_time, tagID, site) %>% 
  mutate(allM2 = ifelse(site == "maisC" & n() == 4, "yes", 
                          ifelse(site == "maisD" & n() == 2, "yes", "no"))) %>% 
  ungroup()

## add site_meth groups
df.t3$group <- paste0(df.t3$site, "_", df.t3$meth)
df.t3$group2 <- paste0(df.t3$meth, "_", df.t3$site)

## add column to indicate whether position is kept due to thresholds
df.t3$thresh <- "yes"
df.t3$thresh[df.t3$meth == "direct.ab" & df.t3$site == "maisC" & df.t3$Ac == df.t3$Sc] <- "no"
df.t3$thresh[df.t3$meth == "direct.ab" & df.t3$site == "maisD" & df.t3$Ac <= 3 & df.t3$Sc %in% c(1, 2, 3)] <- "no"
df.t3$thresh[df.t3$meth == "omni.ab" & df.t3$site == "maisC" & df.t3$Ac <= 2] <- "no"
df.t3$thresh[df.t3$meth == "omni.ml" & df.t3$site == "maisC" & df.t3$Ac <= 2] <- "no"

## add column to indicate whether position is kept due to thresholds
df.t3$thresh2 <- "yes"
df.t3$thresh2[df.t3$meth == "direct.ab" & df.t3$Ac < 2*df.t3$Sc | df.t3$Sc < 3] <- "no"
df.t3$thresh2[df.t3$meth == "direct.an" & df.t3$Sc < 3] <- "no"
df.t3$thresh2[df.t3$meth == "omni.ab" & df.t3$site == "maisC" & df.t3$Sc < 3] <- "no"
df.t3$thresh2[df.t3$meth == "omni.ml" & df.t3$site == "maisC" & df.t3$Sc < 3] <- "no"

## add column to indicate whether position is kept due to thresholds
df.t3$thresh3 <- "yes"
df.t3$thresh3[df.t3$meth == "direct.ab" & df.t3$Ac < 1.5*df.t3$Sc | df.t3$Sc < 3] <- "no"
df.t3$thresh3[df.t3$meth == "direct.an" & df.t3$Sc < 3] <- "no"
df.t3$thresh3[df.t3$meth == "omni.ab" & df.t3$site == "maisC" & df.t3$Sc < 3] <- "no"
df.t3$thresh3[df.t3$meth == "omni.ml" & df.t3$site == "maisC" & df.t3$Sc < 3] <- "no"

df.t3 <- st_as_sf(df.t3, coords = c("lon", "lat"), crs = crsLL)
...
df.t3 <- st_transform(df.t3, crs = crsPlot) 

df.t3 <- df.t3 %>%
  mutate(keepCd = lengths(st_within(df.t3, coreCd)) > 0,
         keepCo = lengths(st_within(df.t3, coreCo)) > 0,
         keepDd = lengths(st_within(df.t3, coreDd)) > 0)

df.t3$core <- NA
df.t3$core[df.t3$meth %in% c("direct.ab", "direct.an") & df.t3$site == "maisC"] <- df.t3$keepCd[df.t3$meth %in% c("direct.ab", "direct.an") & df.t3$site == "maisC"]
df.t3$core[df.t3$meth %in% c("omni.ab", "omni.ml") & df.t3$site == "maisC"] <- df.t3$keepCo[df.t3$meth %in% c("omni.ab", "omni.ml") & df.t3$site == "maisC"]
df.t3$core[df.t3$meth %in% c("direct.ab", "direct.an") & df.t3$site == "maisD"] <- df.t3$keepDd[df.t3$meth %in% c("direct.ab", "direct.an") & df.t3$site == "maisD"]
...
sum.t3 <- as.data.frame(sum.t3)

sum.t3.2 <- df.t3 %>% group_by(site, meth, Sc, Ac) %>%
  summarise(nPoints = n(),
            mPE = median(pred_mod),
...
gpPE <- ggplot(sum.t3.2[sum.t3.2$meth == "direct.ab",]) + 
  # geom_point(aes(x = Ac, y = PE, group = as.factor(Sc), color = as.factor(Sc)),
  #            data = df.m2[df.m2$meth2 == "direct \nab",],
  #            size = 0.5, alpha = 0.2, pch = 1, position = position_dodge(width = 0.3)) +
...
####5) table predictive performance---------------------------------------------
...
df.t3 %>% group_by(group) %>% 
  summarize(MAE = mean(abs(diff)), MAEq50 = mean(abs(diffq50)), 
            meanPA = mean(pred_mod), meanPA50 = mean(q50), 
            meanPE = mean(PE), meanPE50 = median(PE))
...
df.meth <- left_join(df.meth,
                     df.t3[df.t3$allM2 == "yes",] %>% group_by(group) %>% 
...
df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh == "yes",] %>% group_by(group) %>% 
...
df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh2 == "yes",] %>% group_by(group) %>% 
...
df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh3 == "yes",] %>% group_by(group) %>% 
...
df.meth <- left_join(df.meth,
                     df.t3[df.t3$core == TRUE,] %>% group_by(group) %>% 
...
df.meth <- left_join(df.meth,
                     df.t3 %>% group_by(site, meth) %>% summarize(nPfull = n(), .groups = "drop"), 
                 by = c("site", "meth"))

df.meth <- left_join(df.meth,
                     df.t3[df.t3$allM2 == "yes",] %>% group_by(site, meth) %>% 
                       summarize(nPallM = n(), .groups = "drop"), 
                     by = c("site", "meth"))

df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh == "yes",] %>% group_by(site, meth) %>% 
                       summarize(nPthresh = n(), .groups = "drop"), 
                     by = c("site", "meth"))

df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh2 == "yes",] %>% group_by(site, meth) %>% 
                       summarize(nPthresh2 = n(), .groups = "drop"), 
                     by = c("site", "meth"))

df.meth <- left_join(df.meth,
                     df.t3[df.t3$thresh3 == "yes",] %>% group_by(site, meth) %>% 
                       summarize(nPthresh3 = n(), .groups = "drop"), 
                     by = c("site", "meth"))

df.meth <- left_join(df.meth,
                     df.t3[df.t3$core == T,] %>% group_by(site, meth) %>% 
...
## plot test data --------------------------------------------------------------
df.t <- st_read(dsn = "./output_model/model_animal_m3_maisC_direct.ab.gpkg", layer = "pPE")

df.t$time <- as.numeric(format(df.t$X_time, "%H")) + as.numeric(format(df.t$X_time, "%M"))/60

df.bird <- st_read("./data/animal/Data_animal_handheld_maisC.gpkg")
...
g2 <- ggplot() + 
  ...
  # handheld data
  geom_sf(aes(fill = time), shape = 21, alpha = 0.6, size = 5, color = "white",
          data = df.bird[df.bird$Testtag == "ClGT11",], stroke = 1) +
  geom_sf(aes(fill = time), shape = 24, alpha = 0.6, size = 5, color = "white",
          data = df.bird[df.bird$Testtag == "ClRO08",], stroke = 1) +
  # stations
  ...
  annotation_scale(height = unit(0.4, "cm"), text_cex = 1.5, width_hint = 0.4, 
                   bar_cols = c("grey60", "white"), location = "br") +
  scale_color_viridis_c("time", option = "inferno", end = 0.8, begin = 0.1, direction = 1,
                       labels = c("08:00 am", "12:00 pm", "04:00 pm"), 
                       breaks = c(8, 12, 16), limits = c(6.9, 18.5)) +
  scale_fill_viridis_c("time", option = "inferno", end = 0.8, begin = 0.1, direction = 1,
                        labels = c("08:00 am", "12:00 pm", "04:00 pm"), 
                       breaks = c(8, 12, 16), limits = c(6.9, 18.5)) +
  guides(
    shape = guide_legend(override.aes = list(
      color = c("black", "#bc3754", "white", "#bc3754", "white"),  # fake color for legend symbols
      fill = c("white", NA, "#bc3754", NA, "#bc3754"),    # needed if using filled shapes
      size = c(6, 5, 6, 5, 6),
      stroke = 1,
      alpha = 0.9
    ))) +
