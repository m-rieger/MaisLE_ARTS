...
df$tagheight_m <- factor(df$tagheight_m, ordered = T)
...
var_num <- c("dens", "Station.Count", "Antenna.Count", "Signal.max", "Weight")                              # assign default NA if missing.
...
#==============================================================#
# RANDOM term(s)
var_rand <- c("tagheight_m") # Individual                        # assign default NA if missing.
...
df <- df.pr
table(df$pos, df$AcSc, df$tagheight_m)
...
dispersion_simulation(data = df, 
                      modelTMB = mod,
                      response = var_resp,
                      predictor = var_fac2[3], # (optional, but recommended) - A SINGLE factor predictor to split this plot. Use a different index number as needed.
...
ppcheck_fun(data = df,
            modelTMB = mod,
            response = var_resp,
            predictor = var_fac2[2],   # (optional, but recommended) - A SINGLE factor predictor to split this plot.
            n.sim = 500)                # Number of simulations, set to >2000 for final model assessment.
...
plot_random <- unique(df$tagheight_m) #c("TT090C", "TT163C")                             # Assign NA if missing.
...
newdat <- expand.grid("detR" = factor(unique(df$detR), ordered = T),
                      "pos" = levels(df$pos),
                      "AcSc" = levels(df$AcSc),
                      "dens_z" = mean(df$dens_z),
                      "Station.Count_z" = mean(df$Station.Count_z),
                      "Antenna.Count_z" = mean(df$Antenna.Count_z),
                      "Signal.max_z" = mean(df$Signal.max_z),
                      "Weight_z" = mean(df$Weight_z),
                      "tagheight_m" = factor(unique(df$tagheight_m), ordered = T))
