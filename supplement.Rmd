---
title: "Supplement of 'Enabling High-Resolution Wildlife Tracking: antenna beam-based approach including novel per-position error estimations for Automated Radiotelemetry Systems'"
author: "Rieger, M. R., Schmitt, J., Machin, P., Musculus, J., Dittrich, R., Gottschalk, T. K., Gottwald, J., Lampe, P., & Höchst, J."
...
## Study area
```{r}
kbl(tab.sites, caption = "Properties of the study areas maisC and maisD, including the number of directional (<em>N direct</em>) and omnidirectional (<em>N omni</em>) stations and the mean distance to the closest station (<em>distance</em>), the area enclosed by the station set-up (<em>core area</em>) as well as the number of transmitters used for testing (<em>N tags</em>) and the height above ground they were attached (<em>height tags</em>).",     row.names = F,
      escape = FALSE,
      align = c("lll")) %>%
  ...
  row_spec(9, extra_css = "border-bottom: 1px solid black;")
...
kbl(sum.p,
             caption = "Properties of test tracks used to compare position estimations and errors for maisC (<em>C1-3, Ctest</em>) and maisD (<em>D1-3, Ctest</em>). <em>N points</em> is the number of recorded GPS fixes (summarized in 2-second intervals) and <em>N tags</em> the number of used transmitters.",
      row.names = F,
      escape = FALSE,
      align = c("llrrrr")) %>%
  row_spec(which(1:nrow(sum.p)%%2!=1),
...
knitr::include_graphics("./plots_supplement/bearing-model-crop.png")
```
...
```{r, include=TRUE, fig.align="center", fig.cap = c("Exponential power distance model fit for one directional (*left*) and one omnidirectional (*right*) station in maisC."), out.width = "800px"}
knitr::include_graphics("./plots_supplement/Distance_model.png")
```
...
knitr::include_graphics("./plots_supplement/pos2-intersections.png")
```
...
knitr::include_graphics("./plots_supplement/pos2-antennabeams.png")
```
...
knitr::include_graphics("./plots_supplement/pos2-multilaterations.png")
```
...
## maisD omni (for area of core area)
# antenna beams
polyDoab <- df.stat[df.stat$station.project_id == "maisD" & df.stat$type == "direct",]
polyDoab <- st_convex_hull(st_union(polyDoab))

gC <- ggplot() + 
  annotation_map_tile(type = "osm", alpha = 0.4) +
  geom_sf(data = polyCdan, alpha = 0.3, fill = "white", aes(lty = "direct an"), lwd = 1) +
  geom_sf(data = polyComl, alpha = 0.3, fill = "grey60", aes(lty = "omni ml"), lwd = 1) +
  geom_sf(data = polyCdab, alpha = 0.3, fill = "white", aes(lty = "direct ab"), lwd = 1) +
  geom_sf(data = polyCoab, alpha = 0.3, fill = "grey60", aes(lty = "omni ab"), lwd = 1) +
  ...
  annotation_scale(height = unit(0.4, "cm"), text_cex = 1, width_hint = 0.4, style = "ticks") +
  facet_grid(~"maisC") +
...
gD <- ggplot() + 
  annotation_map_tile(type = "osm", alpha = 0.4) +
  geom_sf(data = polyDdan, alpha = 0.3, fill = "white", aes(lty = "direct an"), lwd = 1) +
  geom_sf(data = polyDdab, alpha = 0.3, fill = "white", aes(lty = "direct ab"), lwd = 1) +
...
```{r}
df.m2$X_time <- as.POSIXct(df.m2$X_time)
df.m2$time <- as.numeric(format(df.m2$X_time, "%H")) + as.numeric(format(df.m2$X_time, "%M"))/60
...
## df.m2 as shp
shp.m4 <- st_as_sf(df.m2, coords = c("lon", "lat"), crs = crsLL)
shp.est <- NULL

for(m in unique(df.m2$meth2)) {
  
  for(t in unique(df.m2$tagID)) {
    s <- unique(df.m2$site[df.m2$meth2 == m & df.m2$tagID == t])
    tmp.p <- left_join(shp.p[shp.p$site %in% s,], df.m2[df.m2$meth2 == m & df.m2$tagID == t, c("X_time", "PE", "lon", "lat", "allM2")],
                   by = c("X_time"))
    tmp.p$meth2 <- m
    tmp.p$tagID <- t
...
```{r, fig.cap = "Correlations between predictors (Ac = number of receiving antennas, Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB), Weight = summed up normalized signals), including correlation coefficients (upper panels) and histograms (diagonal panels) for directional antenna beams in maisC. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "direct.ab" & df.m2$site == "maisC", c("Ac", "Sc", "cover", "maxSig", "Weight", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```

### direct ab - maisD {.unnumbered}

```{r, fig.cap = "Correlations between predictors (Ac = number of receiving antennas, Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB), Weight = summed up normalized signals), including correlation coefficients (upper panels) and histograms (diagonal panels) for directional antenna beams in maisD. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "direct.ab" & df.m2$site == "maisD", c("Ac", "Sc", "cover", "maxSig", "Weight", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```

### direct an - maisC {.unnumbered}

```{r, fig.cap = "Correlations between predictors (Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB)), including correlation coefficients (upper panels) and histograms (diagonal panels) for directional angulation in maisC. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "direct.an" & df.m2$site == "maisC", c("Sc", "cover", "maxSig", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```

### direct an - maisD {.unnumbered}

```{r, fig.cap = "Correlations between predictors (Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB)), including correlation coefficients (upper panels) and histograms (diagonal panels) for directional angulation in maisD. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "direct.an" & df.m2$site == "maisD", c("Sc", "cover", "maxSig", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```

### omni ab - maisC {.unnumbered}

```{r, fig.cap = "Correlations between predictors (Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB), Weight = summed up normalized signals), including correlation coefficients (upper panels) and histograms (diagonal panels) for omnidirectional antenna beams in maisC. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "omni.ab" & df.m2$site == "maisC", c("Sc", "cover", "maxSig", "Weight", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```

### omni ml - maisC {.unnumbered}

```{r, fig.cap = "Correlations between predictors (Sc = number of receiving stations, cover = station cover (proxy for detection probability), maxSig = maximum received signal (dB)), including correlation coefficients (upper panels) and histograms (diagonal panels) for omnidirectional multilateration in maisC. For comparison, the log10-scaled response variable PE is given as well (PE_log10).", fig.width = 10, fig.height = 5.5}
df <- df.m2[df.m2$meth == "omni.ml" & df.m2$site == "maisC", c("Sc", "cover", "maxSig", "PE_log10")]
pairs(df, upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.point)
```
...
all_levels <- levels(df.sim3$Sc)
fill_values <- color_values <- viridis::viridis(length(all_levels))  # Generates 10 distinct colors

plotPAdab <- function(d.pred, PApred, lower, upper, lower25, upper75, ytitle, xtitle, x, dw) {
  ...
  g <- ggplot(d.pred) + 
    geom_line(aes(x = x, y = PApred, group = as.factor(Sc), color = as.factor(Sc)),
              lwd = 1, alpha = 0.5, position = position_dodge(width = dw)) + 
    geom_linerange(aes(x = x, y = PApred, ymin = lower, ymax = upper,
                        group = as.factor(Sc), color = as.factor(Sc)),
              lwd = 0.5, alpha = 0.7, position = position_dodge(width = dw)) + 
    geom_linerange(aes(x = x, y = PApred, ymin = lower25, ymax = upper75,
                       group = as.factor(Sc), color = as.factor(Sc)),
                   lwd = 1, alpha = 0.7, position = position_dodge(width = dw)) + 
...
plotPA <- function(d.sim, PAsim, ytitle, xtitle, x) {
  ...
  g <- ggplot(d.sim) + 
    ...
    stat_halfeye(aes(x = as.factor(x), y = PAsim, fill = Sc, color = Sc,
...
```{r, fig.cap = "Predicted mean PE (pPE, based on 4000 simulations) from the model, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels) for all present combinations of Ac-Sc per site and method. For conditional posterior predictions, maxSig, (weight) and cover were set to their respective raw data mean per Ac-Sc combination.", fig.height = 12}

g1 <- plotPAdab(d.pred = df.pred3[df.pred3$meth == "direct.ab",], 
       PApred = "pred_mod", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac", dw = 0.4)

g2 <- plotPA(d.sim = df.sim3[df.sim3$meth == "direct.an",], 
       PAsim = "sim.m", 
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac")

g3 <- plotPA(d.sim = df.sim3[df.sim3$meth %in% c("omni.ab", "omni.ml"),], 
       PAsim = "sim.m", 
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac")
...
### pPE Ctest, Dtest {.unnumbered .tabset}

#### Ac {.unnumbered}

```{r, fig.cap = "Predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels). Positiones are summarized per present combinations of Ac-Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3.2[sum.t3.2$meth == "direct.ab",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac", dw = 0.4)

g2 <- plotPA(d.sim = df.t3[df.t3$meth == "direct.an",], 
       PAsim = "pred_mod", 
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac")

g3 <- plotPA(d.sim = df.t3[df.t3$meth %in% c("omni.ab", "omni.ml"),], 
       PAsim = "pred_mod",
       ytitle = "pPE [m]",
       xtitle = "number of antennas", x = "Ac")
...
```{r, fig.cap = "Predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar). Positiones are summarized per present combinations of maximum signal strength (rounded to tens) and Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3.2Sig[sum.t3.2Sig$meth == "direct.ab",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)

g2 <- plotPAdab(d.pred = sum.t3.2Sig[sum.t3.2Sig$meth == "direct.an",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)

g3 <- plotPAdab(d.pred = sum.t3.2Sig[sum.t3.2Sig$meth %in% c("omni.ab", "omni.ml"),], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)
...
```{r, fig.cap = "Predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar). Positiones are summarized per present combinations of summed up normalized signals (rounded to fifties) and Sc per site and method antenna beams. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 8}

g1 <- plotPAdab(d.pred = sum.t3.2W[sum.t3.2W$meth == "direct.ab",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "summed up normalized signals", x = "Weight_c", dw = 20)

g3 <- plotPAdab(d.pred = sum.t3.2W[sum.t3.2W$meth %in% c("omni.ab"),], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "summed up normalized signals", x = "Weight_c", dw = 20)
...
g <- g1 + g3 + plot_layout(design = layout)
g
...
```{r, fig.cap = "Predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar). Positiones are summarized per present combinations of station cover (rounded to ones) and Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3.2C[sum.t3.2C$meth == "direct.ab",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)

g2 <- plotPAdab(d.pred = sum.t3.2C[sum.t3.2C$meth == "direct.an",], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)

g3 <- plotPAdab(d.pred = sum.t3.2C[sum.t3.2C$meth %in% c("omni.ab", "omni.ml"),], 
       PApred = "mPE", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)
...
### diff PE - pPE Ctest, Dtest {.unnumbered .tabset}

#### Ac {.unnumbered}

```{r, fig.cap = "Difference between raw PE and predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median difference (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels). Positiones are summarized per present combinations of Ac-Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3[sum.t3$meth == "direct.ab",], 
...
```{r, fig.cap = "Difference between raw PE and predicted PE (pPE, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median difference (triangle) including 50 % (thick bar) and 95 % CI (thin bar). Positiones are summarized per present combinations of maximum signal strength (rounded to tens) and Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3Sig[sum.t3Sig$meth == "direct.ab",], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)

g2 <- plotPAdab(d.pred = sum.t3Sig[sum.t3Sig$meth == "direct.an",], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)

g3 <- plotPAdab(d.pred = sum.t3Sig[sum.t3Sig$meth %in% c("omni.ab", "omni.ml"),], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "max. signal strength", x = "maxSig_c", dw = 4)
...
g1 <- plotPAdab(d.pred = sum.t3W[sum.t3W$meth == "direct.ab",], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "summed up normalized signals", x = "Weight_c", dw = 20)

g3 <- plotPAdab(d.pred = sum.t3W[sum.t3W$meth %in% c("omni.ab"),], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "summed up normalized signals", x = "Weight_c", dw = 20)
...
g1 <- plotPAdab(d.pred = sum.t3C[sum.t3C$meth == "direct.ab",], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)

g2 <- plotPAdab(d.pred = sum.t3C[sum.t3C$meth == "direct.an",], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)

g3 <- plotPAdab(d.pred = sum.t3C[sum.t3C$meth %in% c("omni.ab", "omni.ml"),], 
       PApred = "mdiff", lower = "lwr", upper = "upr", 
       lower25 = "lwr25", upper75 = "upr75",
       ytitle = "PE - pPE [m]",
       xtitle = "station cover", x = "cover_c", dw = 0.4)
...
### pPE model q50 {.unnumbered}

```{r, fig.cap = "Predicted median PE (pPE50, based on 4000 simulations) from the model, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels) for all present combinations of Ac-Sc per site and method. For conditional posterior predictions, maxSig, (weight) and cover were set to their respective raw data mean per Ac-Sc combination. This could be done for any desired quantile (e.g. 65 % quantile instead of 50 % quantile).", fig.height = 12}
...
### pPE Ctest, Dtest q50 {.unnumbered}

```{r, fig.cap = "Predicted median PE (pPE50, based on 4000 simulations) for position estimates from test tracks *Ctest* and *Dtest*, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels). Positiones are summarized per present combinations of Ac-Sc per site and method. Note that this is based on raw data (observation-level posterior predictions), therefore showing more variance and larger CIs.", fig.height = 12}

g1 <- plotPAdab(d.pred = sum.t3.2[sum.t3.2$meth == "direct.ab",], 
       PApred = "mPEq50", lower = "lwrq50", upper = "uprq50", 
       lower25 = "lwr25q50", upper75 = "upr75q50",
       ytitle = "pPE50 [m]",
       xtitle = "number of antennas", x = "Ac", dw = 0.4)
...
## Position error overview

```{r}

tab.ov <- df.ov
tab.ov$mean[tab.ov$mean == ""] <- NA

colnames(tab.ov) <- c("study", "antenna type", "spacing [km]", "height [m]", "testtag", "area covered", "method", "mean [m]", "median [m]", "DOI")

tab.ov$study <- cell_spec(tab.ov$study, "html", link = tab.ov$DOI)
tab.ov <- select(tab.ov, -DOI)

kbl(tab.ov, caption = "Overview of position errors (PE) of recent studies using ARTS with different position estimation methods in comparison with our study. PEs are given as mean and/or median. The number of directional antennas per station is given in brackets. To set the PE in relation to the used station set-up, average distance between station (spacing), station height, type of testtag procedure (stationary at points or moving along a track), as well as the covered area (core area = within the station set-up, all = including perimeter outside the set-up) are given as well.",     
    row.names = F,
      escape = FALSE,
      align = c("lll")) %>%
  add_header_above(c(" ", "station" = 3, " " = 3, "PE" = 2)) %>%
  row_spec(which(1:nrow(tab.ov)%%2!=1),
           background = "#CCCCCC") %>%
  kable_styling(font_size = 12, latex_options = "hold_position") %>%
  row_spec(0, extra_css = "border-bottom: 1px solid black;", background = "#CCCCCC") %>%
  row_spec(9, extra_css = "border-bottom: 1px solid black;") %>%
...
## Bird data example
We also used data from one Great Tit *Parus major* and one European Robin *Erithacus rubecula* that were tagged in the course of the project to test whether the methods used can also be applied to real data.
We collected the respective ground-truth data with handheld antennas and manual angulation in the field, estimating a position at least every ten minutes for one day.
As there was rarely visual contact with the bird, these positions only served as a rough estimate of where the bird was. 
For the trapping, handling and tagging of birds, authorizations were issued by the State Office for Labor Protection, Consumer Protection and Health, Brandenburg (LAVG, 2347-80-2023-9-G) and the State Office for the Environment, Brandenburg (LfU).
For animal tagging we used the same transmitters as for test tracks and ensured that transmitter weights (0.37 g, 0.6 g) did not exceed 3 % of the animal's body weight.

Visual comparison of position estimation and error prediction with data from a tagged Great Tit and European Robin revealed a close match between positions derived from handheld telemetry and positions derived from our ARTS using directional antenna beams, but positions spread further when estimated with antenna beams.
Further, pPEs were larger for positions that were farther away from the respective handheld positions.

```{r, include=TRUE, fig.align="center", fig.cap = c("Estimated positions using directional antenna beams (empty symbols, size is scaled to pPE) and positions located with handheld antennas (filled symbols) recorded in one day for two individuals in maisC (Great Tit, European Robin)."), out.width = "800px"}
knitr::include_graphics("./plots_supplement/plotAnimal.png")
```
