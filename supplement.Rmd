...
knitr::include_graphics("./plots/Distance_model.png")
```
...
```{r, fig.cap = "Predicted median PE (pPE50, based on 4000 repetitions) from the global model, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels) for all present combinations of Ac-Sc per site and method. For predictions, maxSig, (weight) and cover were set to their respective raw data mean per Ac-Sc combination. This could be done for any desired quantile (e.g. 65 % quantile instead of 50 % quantile).", fig.height = 12}
