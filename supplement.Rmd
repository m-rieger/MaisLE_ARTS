...
```{r, fig.cap = "Difference between raw PE and predicted PE (pPE, based on 4000 repetitions) from the global model based on test tracks Ctest and Dtest, namely the median pPE (triangle) including 50 % (thick bar) and 95 % CI (thin bar), as well as the distribution (polygon, only center and bottom panels). Positiones are summarized per present combinations of Ac-Sc per site and method. Note that this is based on raw data (not mean predictions), therefore showing more variance and larger CIs.", fig.height = 12}
